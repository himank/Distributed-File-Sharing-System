...
public class Simpella implements Runnable
{
	   ...
	   while(it.hasNext()) {
			...
			while(lit.hasNext()) {
				FileDetails fd = lit.next();
				 System.out.println(fd.index +")  "+shm.msgFromIp+":"+shm.msgFromPort+"   Size: "+(float)fd.fileSize/1024+"kb");
				   System.out.println("File Name: "+fd.fileName);
	    			...
	    			public boolean accept(File directory, String fileName) 
	    			{
	    			  	...
	    			  	while(i<fileNameSplit.length) { 
	    			  		...
	    			  		if(fileNameSplit[i].contains(".") && !searchquery.contains(".")) {
	    			  			...
	    			  			if(fileNameSplit[i].toLowerCase().equals(searchquery.substring(0, searchquery.indexOf(".")).toLowerCase()))
	    			  				return true;
	    			  			else
	    			  				i++;
	    			  		}else {
   ...
   public void info(String str)
   {
	   if(str.equalsIgnoreCase("q")) {
		   ...
		   System.out.println("Bytes Recevied: "+(float)bytesRecv/1024+"kb  Bytes Sent: "+(float)bytesSent/1024+"kb");
		   System.out.println();
		   ...
		   ListIterator<PeerData> li = connectionList.listIterator();
	   		while(li.hasNext()) {
	   			int i=1;
	   			PeerData p = li.next();
	   			System.out.println(i+") "+p.ip+":"+p.port+"  Packs: "+p.packSent+":"+p.packRecv+"  Bytes: "+(float)p.bytesSent/1024+"kb:"+(float)p.bytesRecv/1024+"kb");
	   		}
	   		System.out.println();
   		}
   ...
   public static void main(String [] args)
   {
      try {
         if(args.length == 2) {
    		  if((Integer.parseInt(args[0]) < 60000) && (Integer.parseInt(args[0]) > 0) && (Integer.parseInt(args[1]) > 0) && (Integer.parseInt(args[1]) <60000)) {
